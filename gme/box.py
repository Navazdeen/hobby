...
class Box():
    ...
    def create_block(self) -> None:
        for x in range(self.rows * self.cols):
            row = x//self.rows
            col = x%self.cols
            self.block_list.append(Block(self.width_space * row, self.height_space * col, x))
    ...
    def update_block(self, dispsurf=None, pygame=None) -> None:
        ...
        for block in self.block_list:
            # print(block.color)
            pygame.draw.rect(dispsurf, block.color, (block.x, block.y, self.width_space-self.line_thick, self.height_space-self.line_thick))
